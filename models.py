...
def create_db_file(db_path: str) -> None:
    """
...
def save_model_predictions_to_db(agent: BankAgent, db_path: str) -> None:
    """
    Generates a mapping between input state values and the model's predicted interest rate,
    then saves this mapping into an SQLite database.
    
    :param agent: The trained bank agent.
    :param db_path: Path to the SQLite database file.
    """
    ...
    defaults = config.DEFAULT_STATE.copy()
    features = ['wealth', 'risk_tolerance', 'loyalty', 'inflation', 'gdp_growth', 'euribor']
    for idx, feature in enumerate(features):
        grid_range = config.PREDICTION_GRID_RANGES[feature]
        for value in grid_range:
            state = defaults.copy()
            state[idx] = value
            predicted_rate = agent.predict_rate(state)
            cursor.execute('''
                INSERT INTO predictions (wealth, risk_tolerance, loyalty, inflation, gdp_growth, euribor, predicted_rate)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (state[0], state[1], state[2], state[3], state[4], state[5], predicted_rate))
    
    conn.commit()
    conn.close()
    print(f"Predictions saved to {db_path}")
